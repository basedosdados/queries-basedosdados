...
select
    ...
    safe_cast(substr(cast(dt_atual), 1, 4) as int64) as ano_atualizacao,
    safe_cast(substr(cast(dt_atual), 5, 2) as int64) as mes_atualizacao,
    ...
    safe_cast(
        safe.parse_date(
            '%Y%m%d', cast(dt_publm as string) as date
        ) data_publicacao_contrato_municipal,
        safe_cast(
            safe.parse_date(
                '%Y%m%d', cast(dt_puble as string) as date
            ...
            safe_cast(
                safe.parse_date(
                    '%Y%m%d', cast(dt_exped as string) as date
