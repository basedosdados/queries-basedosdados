...
WITH empenho_ce AS (
  ...
  LEFT JOIN basedosdados-sating.world_wb_mides_staging.aux_municipio_ce m ON e.codigo_municipio = m.codigo_municipio
),
  pago_ce AS (
    SELECT
    ...
    FROM basedosdados-sating.world_wb_mides_staging.raw_pagamento_ce p
    LEFT JOIN basedosdados-sating.world_wb_mides_staging.aux_municipio_ce m ON p.codigo_municipio = m.codigo_municipio
),
  frequencia_ce AS (
    SELECT
  ...
  FROM basedosdados-sating.world_wb_mides_staging.raw_pagamento_mg AS p
  LEFT JOIN basedosdados-sating.world_wb_mides_staging.raw_rsp_mg AS r ON p.id_rsp=r.id_rsp
),
pago_pb AS (
    SELECT
    ...
    FROM basedosdados-sating.world_wb_mides_staging.raw_pagamento_pb p
    LEFT JOIN basedosdados-sating.world_wb_mides_staging.raw_empenho_pb e ON p.nu_Empenho = e.nu_Empenho AND p.cd_UGestora = e.cd_ugestora AND p.de_UOrcamentaria = e.de_UOrcamentaria AND p.dt_Ano = e.dt_Ano
    LEFT JOIN basedosdados-sating.world_wb_mides_staging.aux_municipio_pb m ON SAFE_CAST(e.cd_ugestora AS STRING) = SAFE_CAST(m.id_unidade_gestora AS STRING)
),
  frequencia_pb AS (
  ...
  pagamento_pe AS (
    SELECT
      ...
      ROUND(SAFE_CAST (0 AS FLOAT64),2) AS valor_liquido_recebido,
    FROM basedosdados-sating.world_wb_mides_staging.raw_pagamento_pe p
    INNER JOIN basedosdados-sating.world_wb_mides_staging.aux_municipio_pe m ON SAFE_CAST(p.ID_UNIDADE_GESTORA AS STRING) = SAFE_CAST(m.ID_UNIDADEGESTORA AS STRING)
),
  pagamento_pr AS (
      SELECT
    ...
    ROUND(SAFE_CAST (0 AS FLOAT64),2) AS valor_liquido_recebido,
  FROM basedosdados-sating.world_wb_mides_staging.raw_pagamento_pr p
  LEFT JOIN basedosdados-sating.world_wb_mides_staging.raw_empenho_pr e ON p.idEmpenho = e.idEmpenho
