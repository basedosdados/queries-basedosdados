...
METADADOS_URL = "https://servicodados.ibge.gov.br/api/v3/agregados/289/metadados"

def get_metadados_categorias(row):
    codigo = row["codigo"]
    
    if codigo.count(".") == 0:
        row["tipo_produto"] = df_metadados.loc[df_metadados["codigo"] == codigo, "nome"].iloc[0]
    else:
        row["produto"] = df_metadados.loc[df_metadados["codigo"] == codigo, "nome"].iloc[0]
        if codigo.split(".")[0] in df_metadados["codigo"].tolist():
            row["tipo_produto"] = df_metadados.loc[df_metadados["codigo"] == codigo.split(".")[0], "nome"].iloc[0]    
    return row
...
df_metadados_enriquecidos.to_csv('extracao_vegetal_metadados_enriquecidos.csv', index=False)
