...
if __name__ == '__main__':
    ...
    ARQUIVOS_JSON = list(Path('./output/extracao_vegetal/json/').glob('*.json'))
    JSON_FALTANTES = [arquivo for arquivo in ARQUIVOS_JSON if int(arquivo.stem) not in ANOS_TRANSFORMADOS]

    df_metadados_enriquecidos = pd.read_csv('extracao_vegetal_metadados_enriquecidos.csv', dtype=str)
    ...
    for json_path in JSON_FALTANTES:
        ...
        output_path = Path(f'./output/extracao_vegetal/parquet/ano={json_path.stem}')
