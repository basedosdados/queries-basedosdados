...
CATEGORIAS =    pd.read_csv('extracao_vegetal_metadados_enriquecidos.csv', dtype=str)['id'].tolist()
BAIXADOS =      [int(x.stem) for x in Path('./output/extracao_vegetal/json/').glob('*.json')]
...
async def main(years, categories):
    for year in years:
        ...
        async with aiohttp.ClientSession(connector=TCPConnector(limit=50, force_close=True), timeout=ClientTimeout(total=600)) as session:
            tasks = []
